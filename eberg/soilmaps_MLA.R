...
## http://gsif.isric.org/doku.php/wiki:soilmapping_using_mla
## tom.hengl@isric.org

list.of.packages <- c("nnet", "plyr", "ROCR", "randomForest", "plyr", "parallel", "psych", "mda", "Cubist", "h2o", "dismo", "grDevices", "snowfall", "hexbin", "lattice", "ranger", "xgboost", "doParallel", "caret") ## "mxnet", "bartMachine"
...
source("soilmap_functions.R")

## Load Ebergotzen data set ----
set.seed(42)
data(eberg)
data(eberg_grid)
...
## Visual comparison:
plot(log1p(raster(edgeroi.grids["XGBoost"])), col=SAGA_pal[[1]], zlim=log1p(c(5,65)))
points(edgeroi.sp, pch="+", col="black")
...
ORCDRC.mv <- join(ORCDRC.mv, as.data.frame(edgeroi.sp)[,c("SOURCEID","LONGDA94","LATGDA94")], type="left", match = "first")
