...
legend("topleft", rev(rx), fill=rev(SAGA_pal[[1]]), horiz=FALSE, bty="n", cex=0.8, y.intersp=.2)
image(raster(pr.mw2@predicted[2]), col=SAGA_pal[[1]], main="Clay %", asp=1, zlim=c(0,65), axes=FALSE, xlab="", ylab="")
legend("topleft", rev(rx), fill=rev(SAGA_pal[[1]]), horiz=FALSE, bty="n", cex=0.8, y.intersp=.2)
image(sqrt(raster(pr.mw1@predicted[1])), col=SAGA_pal[["SG_COLORS_YELLOW_RED"]], main="Error (weighted)", asp=1, zlim=sqrt(c(0,830)), axes=FALSE, xlab="", ylab="")
points(carson.sp[[1]], pch="+")
legend("topleft", rev(rx2), fill=rev(SAGA_pal[["SG_COLORS_YELLOW_RED"]]), horiz=FALSE, bty="n", cex=0.8, y.intersp=.2)
image(sqrt(raster(pr.mw2@predicted[1])), col=SAGA_pal[["SG_COLORS_YELLOW_RED"]], main="Error", asp=1, zlim=sqrt(c(0,830)), axes=FALSE, xlab="", ylab="")
points(carson.sp[[1]], pch="+")
legend("topleft", rev(rx2), fill=rev(SAGA_pal[["SG_COLORS_YELLOW_RED"]]), horiz=FALSE, bty="n", cex=0.8, y.intersp=.2)
